...
func _on_empty_button_input_event(viewport: Node, event: InputEvent, shape_idx: int) -> void:
	if mode == Mode.ACTIVE && CursorManager.current_cursor == CursorManager.CURSOR:
		if event.is_action_pressed("click") && !event.is_action_pressed("pan"):
			$DesktopWindows/RecyclingBin/EmptyButton.scale = Vector2(1.10, 1.10)
			#$DesktopWindows/Antivirus/SeemsFine.text = "scanning..."
			#$DesktopWindows/Antivirus/VirusScanTimer.start()
	
		if event.is_action_released("click"):
			$DesktopWindows/RecyclingBin/EmptyButton.scale = Vector2(1.05, 1.05)

func _on_empty_button_mouse_entered() -> void:
	if mode == Mode.ACTIVE && CursorManager.current_cursor == CursorManager.CURSOR:
		$DesktopWindows/RecyclingBin/EmptyButton.scale = Vector2(1.05, 1.05)
